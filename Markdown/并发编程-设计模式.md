...
让有限的工作线程，来轮流异步处理无限多的任务，也可以将其归类于分工模式，它的典型实现就是线程池，以体现了经典设计模式中的享元模式
...
- 过小不能充分利用系统资源，容易导致饥饿
- 过大会导致过多的线程上下文切换，占用过多系统资源

#### 3.1 CPU密集型

通常采用 CPU核数 + 1，保证当线程暂停时，额外的线程能够顶上去，保证CPU时钟周期不被浪费

#### 3.2 IO密集型

线程数 = CPU核数 * 期望CPU利用率 *  总时间（CPU计算时间 + 等待时间）/ CPU计算时间

![image-20220417180531876](\picture\image-20220417180531876.png)
