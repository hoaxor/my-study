    ...
    private final void addCount(long x, int check) {
        ...
        if (check >= 0) {
            ...
            while (s >= (long)(sc = sizeCtl) && (tab = table) != null &&
                ...
                // 相对容量生成唯一的扩容戳，相同容量下戳相等
                int rs = resizeStamp(n);
                // sc小于0说明在扩容中
                if (sc < 0) {
                    // sc >>> RESIZE_STAMP_SHIFT!=rs 表示比较高 RESIZE_STAMP_BITS 位生成戳和 rs 是否相等
                    // 如果不相等则说明已经扩容完成（相对容量生成唯一的扩容戳，相同容量下戳相等）
                    // sc=rs+1 表示扩容结束
                    // sc==rs+MAX_RESIZERS 表示扩容线程数已经达到最大值了
                    // nt=nextTable -> 表示扩容已经结束
                    // transferIndex<=0 表示所有的 transfer 任务都被领取完了，没有剩余的 hash 
                    // 桶来给自己这个线程来做 transfer
                    if ((sc >>> RESIZE_STAMP_SHIFT) != rs || sc == rs + 1 ||
                        sc == rs + MAX_RESIZERS || (nt = nextTable) == null ||
                        transferIndex <= 0)
                        // 5 个条件只要有一个条件为 true，说明当前线程不能帮助进行此次的扩容，直接跳出循环
                        break;
                    // newTable已经创建，帮助转移
                    // cas 扩容线程计数+1
                    ...
                    // sc >=0 表明
                    // 没有在扩容，那么 rs 肯定是一个正数，通过 rs<<RESIZE_STAMP_SHIFT 
                    // 将 sc 设置 为一个负数，+2 表示有一个线程在执行扩容
                    // sc 高 16 位代表扩容戳、低 16 位代表并行扩容的线程数
                    // 这样来存储有什么好处呢?
...
————————————————
版权声明：本文为CSDN博主「寂夜了无痕」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/a15835774652/article/details/121431340*/
                    transfer(tab, null);
                s = sumCount();
            }
    ...
    private final void fullAddCount(long x, boolean wasUncontended) {
        ...
        for (;;) {
            ...
            if ((as = counterCells) != null && (n = as.length) > 0) {
                ...
                if ((a = as[(n - 1) & h]) == null) {
                    // cells对应的索引位置为空
                    // cellsBusy这个字段保证数组新增节点，扩容只有一个线程在进行，防止多线程并发
                    ...
                    // 这里是防止数组一直扩容使用的，数组的最大容量就是CPU的核数
...
```java
    ...
    static final int resizeStamp(int n) {
        // Integer.numberOfLeadingZeros返回无符号整数 n 最高位非 0 位前面的 0 的个 数
        // 比如 10 的二进制是 0000 0000 0000 0000 0000 0000 0000 1010那么这个方法返回的值就是 28
        // 假如 n=16 得到 1000000000011011
...
```java
    ...
    private final void transfer(Node<K,V>[] tab, Node<K,V>[] nextTab) {
        ...
        // 死循环,i 表示桶下标，bound 表示当前线程可以处理的当前桶区间最小下标，死循环的作用是保证拷贝全部完成。
        for (int i = 0, bound = 0;;) {
            //f第一个node
            //fh第一个node的hash
            Node<K,V> f; int fh;
            // 如果当前线程可以向后推进；这个循环就是控制 i 递减。同时，每个线程都会进入这里取得自己需要转移的桶的区间
            // 这个循环只是用来控制每个线程每轮最多copy/transfer的桶的个数，
            ...
            while (advance) {
                ...
                // 或者完成了，修改推进状态为 false，不能推进了。此轮转移任务成功后修改推进状态为 true。
                // 第一次进入循环，i-- 这个判断会无法通过，从而走下面的cas nextIndex 赋值操作（获取最新的转移下标）。
                
                // --i 表示下一个待处理的桶下标，如果它超过分配的,表示当前线程已经分配过的桶区域（范围）
                ...
                // 通过 cas 来修改 TRANSFERINDEX,为当前线程分配任务，
                // 若原table的长度为16处理的节点区间为 (nextBound,nextIndex)->(0,15)
            ...
            // 如果 i >= tab.length(为什么这么判断)
            // 如果 i + tab.length >= nextTable.length  （为什么这么判断）
            if (i < 0 || i >= n || i + n >= nextn) {
                ...
                // https://blog.csdn.net/a15835774652/article/details/121431340
                if (U.compareAndSwapInt(this, SIZECTL, sc = sizeCtl, sc - 1)) {
                   // 如果 sc - 2 不等于标识符左移 16 位。如果他们相等了
                ...
                synchronized (f) {
                    if (tabAt(tab, i) == f) {
                        ...
                        //fh >=0 代表是普通node，非红黑树
                        if (fh >= 0) {
                            // 因为table的大小只能是2的n次方，扩容后，key的下标只能是i 或 i + n
                            // 对老长度进行与运算（第一个操作数的的第n位于第二个操作数的第n位如果都是1，
                            // 那么结果的第n为也为1，否则为0）
                            // 由于 Map 的长度都是 2 的次方（000001000 这类的数字），
                            // 那么hash & oldTable.length 只有 2 种结果，一种是 0，一种是1
                            // 如果是结果是0 ，Doug Lea 将其放在原来的位置，反之放在i + n
                            int runBit = fh & n;
                            Node<K,V> lastRun = f;
                            for (Node<K,V> p = f.next; p != null; p = p.next) {
                                int b = p.hash & n;
                                if (b != runBit) {
                                    // 当前node 和上一个 node在newTable中的下标不一样
                                    // 更新
                                    runBit = b;
                                    lastRun = p;
                                }
                            ...
                            // 如果红黑树的节点数小于等于 6，那么转成链表，反之，创建一个新的树
...
扩容后该HashEntry对应到新数组中的index只可能为i或者i+capacity
...
未加锁，使用UNSAFE保证多线程的可见性，扩容过程中，get先从旧表取内容，扩容完成后，get从新表中取内容
...
计算size前，先不加锁计算两次，如果前后两次结果一样，认为结果正确

如果不一样，进行重试，重试次数超过3，将所有segment锁住，重新计算size
...
public int size() {
  ...
  try {
    for (;;) {
      ...
      // 上一次的所有segment的修改计数之和 等于这次的和则说明segment数组未被修改
      // 跳出循环，返回size
      if (sum == last)
        break;
      last = sum;
...
public class LinkedBlockingQueue<E> extends AbstractQueue<E>
    ...
    static class Node<E> {
         ...
         * - this Node, meaning the successor is head.next  head节点出队时指向自己，
...
```java
    ...
    private void enqueue(Node<E> node) {
        ...
        // 把node 赋给last.next 把node 赋给 last
        // 以下语句相当于last.next = node; last = node; 
        last = last.next = node;
    }
```

![image-20220508223822942](\picture\image-20220508223822942.png)
...
![image-20220508225225129](E:\myCode\my-study\Markdown\picture\image-20220508225225129.png)
...
```java
    ...
    public void put(E e) throws InterruptedException {
        ...
        // 如果队列中有一个元素，唤醒take线程
        if (c == 0)
            // 这里调用的 notEmpty.signal();，而不是 notEmpty.signalAll(); 是为了减少竞争
            signalNotEmpty();
    }
```
...
```java
    public E take() throws InterruptedException {
        ...
        // 如果队列有一个空位，唤醒put线程
        // 多个线程出队，只有一个线程满足 c == capacity
        if (c == capacity)
            // 调用notFull.signal()而不是notFull.signalAll(), 减少竞争
            signalNotFull();
        return x;
    }
```

![image-20220508231154948](\picture\image-20220508231154948.png)
...
public boolean offer(E e) {
    ...
    for (Node<E> t = tail, p = t;;) {
        ...
        // 所以这里需要重新找新的head，因为新的head后面的节点才是激活的节点
            ...
            // p == q 表示 p已经出队，出队时设置把p.next指向p（指向自己）
            // We have fallen off list.  If tail is unchanged, it
            // will also be off-list, in which case we need to
            // jump to head, from which all live nodes are always
            // reachable.  Else the new tail is a better bet.
            // 如果tail 被修改了，p指向新的tail 更好
            // 否则  p 指向head 说明tail也出队了？
            // (t != (t = tail)) 相当于 t != tail; t = tail
            p = (t != (t = tail)) ? t : head;
        else
            // p被更新过 且 tail 被更新了 p指向最新的tail
            // 否则更新p, p 指向 p.next（p的下一个元素）
...
原文链接：https://blog.csdn.net/qq_38293564/article/details/80798310
...
public E poll() {
    ...
    for (;;) {
        ...
        for (Node<E> h = head, p = h, q;;) {
            E item = p.item;
            // 当前节点item不为空，cas 更新item
            // 第一次循环item一定为null，第一个为哑元，哑元item = null
            if (item != null && p.casItem(item, null)) {
                // Successful CAS is the linearization point
                // for item to be removed from this queue.
                // 当前节点不是头节点
                // 并不是每次出队时都更新head节点，当head节点里有元素时，直接弹出head节点里的元素，
                // 而不会更新head节点。只有当head节点里没有元素时，出队操作才会更新head节点。
                // 采用这种方式也是为了减少使用CAS更新head节点的消耗，从而提高出队效率
                if (p != h) // hop two nodes at a time
                    // 更新头节点
                    updateHead(h, ((q = p.next) != null) ? q : p);
                return item;
            }
            // p的下一个节点为null， p是链表中最后一个元素
            else if ((q = p.next) == null) {
                // 头节点更新成p
                updateHead(h, p);
                return null;
            }
            // p.next = p 说明 p 已经出队，重新获取头节点
            else if (p == q)
...
```java
    public int size() {
        int count = 0;
        // first() 找到第一个item不为空的节点
        // succ() 找到p的后继节点或者当p已经出队时返回头节点
...
底层实现了写入时拷贝的思想，增删改操作会将底层数组拷贝一份，更新操作会在新数组上执行，这时不影响其他线程的并发读，读写分离。
